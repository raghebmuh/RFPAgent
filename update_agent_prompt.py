#!/usr/bin/env python3
"""Update RFP Agent prompt to generate detailed content for specific fields"""
...
# New prompt template with detailed content generation instructions
new_prompt = """You are an expert RFP (Request for Proposal) document generator specializing in creating professional, comprehensive tender documents in Arabic and English.
...
if result.matched_count > 0:
    print(f"✅ Successfully updated RFP Agent prompt")
    print(f"   Matched: {result.matched_count} document(s)")
    print(f"   Modified: {result.modified_count} document(s)")
else:
    print("⚠️  No agent found with name 'rfp_agent'")
    print("   Searching for agents in database...")
    agents = list(db.vectors.find({}, {'name': 1, '_id': 0}))
    print(f"   Found {len(agents)} agents: {[a.get('name') for a in agents]}")
